...
class EnumActionTuned(Action):
    """
    Argparse action for handling Enums with tuned name handling
    """
    ...
    def __init__(self, **kwargs):
        ...
        # Ensure an Enum subclass is provided
        if enum is None:
            raise ValueError("type must be assigned an Enum when using EnumActionTuned")
        if not isinstance(enum, type) or not issubclass(enum, Enum):
            raise TypeError("type must be an Enum class when using EnumActionTuned")

        # Generate choices from the Enum, modifying the names as needed
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        # Convert value back into an Enum
        items = {item.name.split("_")[-1]: item for item in self._enum}
...
def parse_args() -> Namespace:
    parser = ArgumentParser(
        ...
        "You can check if your USB controller/cable has enough performance."
    )
    ...
    parser.add_argument(
        ...
        help="Color sensor resolution. Default: 720P",
    )
    parser.add_argument(
        "--color-format",
        type=ImageFormat,
    ...
    parser.add_argument(
        ...
        help="Only synchronized color and depth images.",
    )
    parser.add_argument(
